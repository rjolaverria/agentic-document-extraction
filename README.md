...
PaddleOCR-VL runs entirely in-process via the `paddleocr` and `paddlepaddle` packages. No system-level OCR binaries are required.

Optional GPU usage is controlled by environment variables (e.g., `ADE_PADDLEOCR_USE_GPU=true`) and depends on installing the appropriate PaddlePaddle GPU build.
